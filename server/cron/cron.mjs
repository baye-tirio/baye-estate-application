...
const URL = "https://baye-estate-application.onrender.com";

const getRequest = () => {
  https
    .get(URL, (res) => {
      if (res.statusCode === 200) {
        console.log(`GET request sent successfully to ${URL}`);
      } else {
...
// send a get request at the domain after every one minute to ensure that the domain does not start lagging
const job = new cron.CronJob("*/1 * * * *", getRequest);
